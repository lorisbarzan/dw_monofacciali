...
(defun c:gestparametri ()
  (setq origin (getfiled "Selezionare file Parametri da utilizzare" (substr (findfile "prese.txt") 1 (- (strlen (findfile "prese.txt"))9))  "txt" 8))
...
(defun c:gestparametri ()
  ...
  (if (/= "PARAMETRI" (STRCASE (substr (vl-filename-base origin) 1 9))) (PROGN (ALERT "File non Valido !!!") (exit)))
  (setq nome (substr (vl-filename-base origin) 10))
  (setq filer (open (strcat (substr (findfile "prese.txt") 1 (- (strlen (findfile "prese.txt"))9)) origin) "r"))
  (setq filew (open "c:\\dwdati\\parametri.txt" "w"))
  (while (setq r (read-line filer))
    (if (/= 'NomeParametri (car (read (strcat "(" r ")")))) (write-line r filew)))
  (write-line (strcat "NomeParametri" (chr 9) "\"" nome "\"") filew)
...
(defun c:salvaparametri ()
  ...
  (vl-file-delete (strcat  (substr (findfile "prese.txt") 1 (- (strlen (findfile "prese.txt"))9)) "parametri" nome ".txt"))
  (vl-file-copy "c:\\dwdati\\parametri.txt" (strcat  (substr (findfile "prese.txt") 1 (- (strlen (findfile "prese.txt"))9)) "parametri" nome ".txt"))
  (alert (strcat "Parametri salvati con il nome:  "(substr (findfile "prese.txt") 1 (- (strlen (findfile "prese.txt"))9)) "parametri" nome ".txt"))
  )
